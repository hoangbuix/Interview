...
export class CompanyService {
    ...
    async updateCompany(id: string,update: UpdateCompanyDto): Promise<CompanyModel>{
        const company = await this.companyModel.findByIdAndUpdate({"_id": id, active: true}, {companyName:update.companyName, mentorCompany: update.mentorCompany, description: update.description, startDate: update.startDate, endDate: update.endDate, active: update.active, updatedAt: new Date()}, {new: true}).exec().catch((err) => {
